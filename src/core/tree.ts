...
import { createTreeNodeValue } from './treeNodeValue'
...
export class TreeNode {
   ...
   */
  setCustomRouteBlock(filePath: string, routeBlock: CustomRouteBlock | undefined) {
    this.value.setOverride(filePath, routeBlock)
  }

  getSortedChildren() {
    return Array.from(this.children.values()).sort((a, b) => a.path.localeCompare(b.path))
  }
  ...
  remove(path: string) {
    ...
    const child = this.children.get(segment)
    if (!child) {
      throw new Error(`Cannot Delete "${path}". "${segment}" not found at "${this.path}".`)
    }
