...
export interface RouteRecordOverride extends Partial<Pick<RouteRecordRaw, 'meta' | 'props' | 'alias' | 'path'>> {
  ...
  get overrides() {
    return [...this._overrides.entries()]
      .sort(([nameA], [nameB]) =>
        nameA === nameB
          ? 0
          : nameA !== EDITS_OVERRIDE_NAME && (nameA < nameB || nameB === EDITS_OVERRIDE_NAME)
            ? -1
            : 1,
      )
...
export class TreeNodeValueStatic extends _TreeNodeValueBase {
  ...
  constructor(rawSegment: string, parent: TreeNodeValue | undefined, pathSegment = rawSegment) {
...
export type TreeNodeValue = TreeNodeValueStatic | TreeNodeValueParam | TreeNodeValueGroup
 ...
 */
function resolveTreeNodeValueOptions(options: TreeNodeValueOptions): Required<TreeNodeValueOptions> {
  return {
    format: 'file',
    dotNesting: true,
    ...options,
...
export function createTreeNodeValue(
  ...
  if (params.length) {
    return new TreeNodeValueParam(segment, parent, params, pathSegment, subSegments)
  }

  return new TreeNodeValueStatic(segment, parent, pathSegment)
}

enum ParseFileSegmentState {
  static,
...
function parseFileSegment(
  ...
  if (state === ParseFileSegmentState.param || state === ParseFileSegmentState.paramOptional) {
 ...
 * @returns - the pathSegment and the params
 */
function parseRawPathSegment(segment: string): [string, TreeRouteParam[], SubSegment[]] {
  let buffer = ''
