import type { EditableTreeNode } from './core/extendRoutes.js'
...
export function resolveOverridableOption<T>(defaultValue: T, value?: _OverridableOption<T, T>): T {
  return typeof value === 'function' ? (value as (existing: T) => T)(defaultValue) : (value ?? defaultValue)
}
...
export const DEFAULT_OPTIONS = {
  output: 'src/router/routes.gen.ts',
  extensions: ['.vue'],
  exclude: [],
...
export function resolveOptions(options: Options) {
  ...
  const routesFolder = normalizeRoutesFolderOption(options.routesFolder || DEFAULT_OPTIONS.routesFolder).map(
    (routeOption) => ({
      ...routeOption,
